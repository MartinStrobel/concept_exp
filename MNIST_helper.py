...
def copy_save_image(x,f1,f2,a,b):
  ...
  # open the image
  print(x.shape)
  Image1 = Image.fromarray(x.astype('uint8'),mode='L')
  Image1.save(f1)
...
def load_xyconcept(n, pretrain):
  ...
  concept = np.load(model_dir+'concept_data.npy')
  y = np.zeros((n, 15))
...
def load_model_stm_new(x_train, y_train, x_val, y_val, width=28, \
               height=28, channel=1, pretrain=True):
  ...
  if pretrain:
    ...
    _ = mlp.fit(x_train,y_train,epochs=10,verbose=0)
